...
class LoopSimplifier:
    ...
    def vote(self, changes: list[Change]) -> dict[str, float]:
        ...
        for change in changes:
            ...
            if any(keyword in change.description.lower() for keyword in 
                   ["loop", "comprehension", "enumerate", "built-in"]):
    ...
    def optimize_loops(self, code: str) -> list[Change]:
        ...
        while i < len(lines):
            ...
            elif (re.search(r"for\s+(\w+)\s+in\s+(\w+):", line) and 
                  i + 1 < len(lines) and re.search(r"\s*if\s+", lines[i + 1])):
            ...
            elif (re.search(r"(\w+)\s*=\s*0", line) and 
                  i + 1 < len(lines) and "for " in lines[i + 1]):
                
                sum_match = re.search(r"(\w+)\s*=\s*0", line)
                for_match = re.search(r"for\s+(\w+)\s+in\s+(\w+):", lines[i + 1])
                
                if (sum_match and for_match and i + 2 < len(lines) and 
                    sum_match.group(1) + " +=" in lines[i + 2]):
                ...
                for i in range(1, len(lines)):
                    ...
                    if inner_match:
                        ...
                        for j in range(i + 1, len(lines)):
                            if ".append(" in lines[j]:
                                append_match = re.search(r"(\w+)\.append\((.+)\)", lines[j])
                                if append_match:
